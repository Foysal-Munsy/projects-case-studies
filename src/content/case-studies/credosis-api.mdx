...
timeframe: "2026"
liveUrl: "https://credosis.com/contact#book"
tech:
  - ".NET 8"
  - "ASP.NET Core"
  - "EF Core"
...
export const img = (name) =>
  `${base.replace(/\/$/, "")}/images/credosisapi/${name}`;

## What it is

I built CredosisApi as part of the engineering team at
[Credosis](https://www.credosis.com), the organization behind
[credosis.com](https://www.credosis.com). It's the backend for two things: a
public meeting scheduler on the Credosis marketing site, and the internal admin
console the team uses to run it. The live booking flow it powers is on the
Credosis site at [credosis.com/contact#book](https://credosis.com/contact#book).
...
Here's what that looks like on the public site - a visitor picks a highlighted
day, chooses an available time, then confirms with their details:

<div class="cs-figure-row">
  <figure class="cs-figure">
    <img
      src={img("booking-calendar.png")}
      alt="Booking calendar showing a month with available days highlighted"
      loading="lazy"
    />
    <figcaption>
      Month view - only days with open slots are highlighted.
    </figcaption>
  </figure>
  <figure class="cs-figure">
    <img
      src={img("booking-times.png")}
      alt="Selected day expanded to show the available time slots"
      loading="lazy"
    />
    <figcaption>Pick a day to see its available times.</figcaption>
  </figure>
  <figure class="cs-figure">
    <img
      src={img("booking-form.png")}
      alt="Booking form asking for name, email, optional phone and notes, with a confirm button"
      loading="lazy"
    />
    <figcaption>Enter details and confirm the booking.</figcaption>
  </figure>

</div>

These are the public, visitor-facing screens the API serves. The internal admin
console - where the team sets availability, generates slots, and manages
bookings - is intentionally not shown here for organization security reasons.
...
bookings. So the generator handles each case on purpose: future _unbooked_ slots
from a changed rule get removed, future _booked_ slots get deactivated (the
